          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {foods.map(food => (
              <div key={food._id} className="bg-white text-black rounded-lg shadow-md p-3 hover:shadow-lg transition transform hover:scale-105 flex flex-col items-center">
                ...
                <button onClick={() => addToCart(food)} className="mt-2 bg-blue-500 hover:bg-blue-700 px-3 py-1 rounded text-white transition">Add to Cart</button>
              </div>
            ))}
          </div>
        )}
        {cart.length > 0 && (
          <div className="mt-6 p-4 bg-black bg-opacity-50 rounded-lg shadow-lg">
            ...
            <ul className="mt-2 space-y-2">
              {cart.map((item, index) => (
                <li key={index} className="bg-gray-700 p-2 rounded-lg">{item.name} - Quantity: {item.quantity}</li>
              ))}
            </ul>
            <button onClick={placeOrder} className="mt-4 bg-green-500 hover:bg-green-700 px-4 py-2 rounded transition">Place Order</button>
          </div>
        )}
